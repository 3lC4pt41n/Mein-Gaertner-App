-- Remove broad raw discovery_events read access.
--
-- Privacy contract: exact discovery coordinates and user ids must never be
-- exposed across users. Public features should use SECURITY DEFINER RPCs that
-- return only aggregated or scoped data.

BEGIN;

DROP POLICY IF EXISTS "authenticated_read_discovery_events" ON public.discovery_events;
