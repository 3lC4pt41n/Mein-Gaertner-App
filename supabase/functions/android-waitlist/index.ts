...
const GOOGLE_DIRECTORY_SCOPE = 'https://www.googleapis.com/auth/admin.directory.group.member';
...
> = {
  ...
  en: {
    ...
    userNote: 'If Google Play still needs to sync, try the link again after a few minutes.',
  ...
  it: {
    ...
    userNote: 'Se Google Play deve ancora sincronizzarsi, riprova il link tra qualche minuto.',
  ...
  tr: {
    ...
    userNote: 'Google Play henuz senkronize olmadiysa baglantiyi birkac dakika sonra tekrar dene.',
  ...
  nl: {
    ...
    userNote: 'Als Google Play nog synchroniseert, probeer de link over een paar minuten opnieuw.',
  ...
  da: {
    ...
    userNote: 'Hvis Google Play stadig synkroniserer, saa proev linket igen om et par minutter.',
  ...
  pl: {
    ...
    userNote: 'Jesli Google Play nadal sie synchronizuje, sproboj ponownie za kilka minut.',
    userFooter: 'Jesli to nie Ty wyslales te prosbe, mozesz zignorowac te wiadomosc.',
  },
  uk: {
    ...
    userNote: 'Якщо Google Play ще синхронізується, спробуйте посилання ще раз за кілька хвилин.',
  ...
  hi: {
    ...
    userNote: 'अगर Google Play अभी sync कर रहा है, तो कुछ मिनट बाद लिंक फिर से खोलें।',
  ...
  ja: {
    ...
    userNote: 'Google Playの同期に少し時間がかかる場合は、数分後にもう一度お試しください。',
  ...
  id: {
    ...
    userNote: 'Jika Google Play masih sinkronisasi, coba tautan ini lagi setelah beberapa menit.',
  ...
  ar: {
    ...
    userNote: 'إذا كان Google Play ما زال يزامن، جرّب الرابط مرة أخرى بعد بضع دقائق.',
  ...
  fa: {
    ...
    userFooter: 'اگر شما این درخواست را نداده‌اید، می‌توانید این ایمیل را نادیده بگیرید.',
  },
  ur: {
    ...
    userNote: 'اگر Google Play ابھی sync کر رہا ہے تو چند منٹ بعد دوبارہ کوشش کریں۔',
...
function getGoogleServiceAccount(): GoogleServiceAccount {
  ...
  return {
    ...
    private_key: Deno.env.get('GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY') || undefined,
  };
}
...
function assertAndroidFlowConfigured() {
  ...
  if (!serviceAccount.client_email || !serviceAccount.private_key || !delegatedAdmin) {
    throw new Error(
...
async function addEmailToGoogleGroup(email: string): Promise<'added' | 'already-member'> {
  const { serviceAccount, delegatedAdmin } = assertAndroidFlowConfigured();
  const accessToken = await getGoogleAccessToken(serviceAccount, delegatedAdmin);
  ...
  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Google group member insert failed (${response.status}): ${body}`);
  }
...
function buildAdminHtml(
  ...
  return `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#173420">
      ...
      <p style="margin:0 0 8px;"><strong>Sprache:</strong> ${escapeHtml(language)}</p>
      <p style="margin:0 0 8px;"><strong>Google-Gruppe:</strong> ${escapeHtml(
        ANDROID_GOOGLE_GROUP_EMAIL
      )}</p>
      <p style="margin:0 0 8px;"><strong>Gruppenstatus:</strong> ${escapeHtml(groupStatus)}</p>
      <p style="margin:0 0 8px;"><strong>Invite-Link:</strong> ${escapeHtml(ANDROID_INVITE_URL)}</p>
      <p style="margin:0 0 8px;"><strong>Quelle:</strong> ${escapeHtml(source)}</p>
      <p style="margin:0 0 8px;"><strong>Seite:</strong> ${escapeHtml(pageUrl)}</p>
    </div>
  `;
}
    ...
    <div style="font-family:Arial,sans-serif;line-height:1.7;color:#173420;background:#f7fbf5;padding:24px;">
      <div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:18px;padding:28px;border:1px solid #dbe8d7;">
        ...
        <p style="margin:24px 0;">
          <a href="${safeInviteUrl}" style="display:inline-block;padding:14px 20px;border-radius:999px;background:#1f5e28;color:#ffffff;text-decoration:none;font-weight:700;">${escapeHtml(
            copy.userCta
          )}</a>
        </p>
        <p style="margin:0 0 12px;color:#5e6b62;font-size:14px;">${escapeHtml(copy.userNote)}</p>
        <p style="margin:0 0 18px;color:#5e6b62;font-size:13px;word-break:break-all;">${safeInviteUrl}</p>
        <p style="margin:0;color:#5e6b62;font-size:14px;">${escapeHtml(copy.userFooter)}</p>
      </div>
    </div>
  `;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'Android invite flow not configured' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
  ...
  if (userSend.error) {
    ...
    return new Response(JSON.stringify({ error: 'Could not send invite email' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
